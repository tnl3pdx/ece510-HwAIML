...
def twos_complement(val, bits):
    """Convert unsigned int to signed int (two's complement)."""
    if (val & (1 << (bits - 1))) != 0:
        val = val - (1 << bits)
    return val
...
class Agent:
    ...
    def Q_Learning(self,episodes):
        ...
        rtl_alpha = pyrtl.Input(bits, 'alpha')         # e.g., 0.5 * 256 = 128
        rtl_gamma = pyrtl.Input(bits, 'gamma')         # e.g., 0.9 * 256 = 230
        rtl_reward = pyrtl.Input(bits, 'reward')       # e.g., -5 * 256 = -1280
        rtl_Q_current = pyrtl.Input(bits, 'Q_current') # Q[(i,j,action)] * 256
        rtl_Q_next = pyrtl.Input(bits, 'Q_next')       # Q[nxtStateAction] * 256
        rtl_q_value = pyrtl.Output(bits, 'q_value')
        rtl_one = pyrtl.Const(256, bitwidth=16)

        # (1 - alpha) in Q8.8
        rtl_one_minus_alpha = rtl_one - rtl_alpha
        
        # gamma * Q_next (Q8.8 * Q8.8 = Q16.16)
        rtl_gamma_Q_next = rtl_gamma * rtl_Q_next
        rtl_reward_scaled = rtl_reward * 256
        rtl_reward_plus = rtl_reward_scaled + rtl_gamma_Q_next
        rtl_alpha_reward_plus = rtl_alpha * rtl_reward_plus
        rtl_one_minus_alpha_Q = rtl_one_minus_alpha * rtl_Q_current * 256

        rtl_q_value <<= rtl_one_minus_alpha_Q + rtl_alpha_reward_plus
        ...
        while(x < episodes):
            ...
            if self.isEnd:
                ...
                for a in self.actions:
                    ...
                    sim.step({
                        'alpha': int(self.alpha * 256),
                        'gamma': int(self.gamma * 256),
                        'reward': int(reward * 256),
                        'Q_current': int(self.Q[(i,j,action)] * 256),
                        'Q_next': int(self.Q[nxtStateAction] * 256) 
                    })
                    
                    q_out = sim.inspect(rtl_q_value)
                    #print("The latest value of 'q_value' was: " + str(q_out))
                    
                    q_value = (q_out / float(1 << 24)) - 256
                    
                    if (args.debug):
                        q_cmp = (1-self.alpha)*self.Q[(i,j,action)] + self.alpha*(reward + self.gamma*self.Q[nxtStateAction])
                        print('Q value (Hardware)', q_value)
                        print('Q value (Software):', q_cmp)
